...
/**
 * "Top ranks, GATE 2026" - the All India Ranks from the press clippings, as
 * chips that drift gently at slightly different rhythms.
 *
 ...
 *
 * The chips float on the existing `float` keyframe with staggered delays and
 * durations, so they never move in lockstep. Transform only; frozen under
 * reduced motion.
 */
  ...
  return (
    <div className={`animate-reveal ${className}`} style={{ animationDelay: `${delay}ms` }}>
      ...
      <ul className="mt-2.5 flex flex-wrap gap-2" aria-label={HERO_RANKS_LABEL}>
        {HERO_RANKS.map((rank, i) => (
          <li
            key={rank}
            className="animate-float"
            style={{
              animationDelay: `${-(i * 1.1)}s`,
              animationDuration: `${6 + (i % 3) * 1.2}s`,
            }}
          >
